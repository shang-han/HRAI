...
def _find_bash() -> str:
    ...
    # Project-local portable Git Bash (bundled inside this deployment).
    # local.py lives in <bundle>/hermes-agent/tools/environments/, so
    # parents[3] is the <bundle> root (e.g. resources/hermes).
    try:
        bundled_root = Path(__file__).resolve().parents[3]
        for candidate in (
            bundled_root / "git" / "bin" / "bash.exe",
            bundled_root / "git" / "usr" / "bin" / "bash.exe",
        ):
            if os.path.isfile(candidate) and str(candidate) not in candidates:
                candidates.append(str(candidate))
    except Exception:
        logger.debug("Could not resolve bundled Git Bash path", exc_info=True)
