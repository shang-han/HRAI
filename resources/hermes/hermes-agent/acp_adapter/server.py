...
class HermesACPAgent(acp.Agent):
    ...
    _SLASH_COMMANDS = {
        ...
        "new": "Start a new conversation (clear history and queued prompts)",
        "stop": "Stop the current generation and clear queued prompts",
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "new",
            "description": "Start a new conversation (clear history and queued prompts)",
        },
        {
            "name": "stop",
            "description": "Stop the current generation and clear queued prompts",
        },
        {
            "name": "version",
    ...
    async def cancel(self, session_id: str, **kwargs: Any) -> None:
        ...
        if state and state.cancel_event:
            with state.runtime_lock:
                ...
                # 与消息渠道的 /stop 一致：取消当前回合时也清掉已排队消息。
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            # 即使后处理（保存会话/发送通知等）抛异常，也必须把会话复位为空闲，
            # 否则后续 prompt 会一直误判为运行中而被排队。
            with state.runtime_lock:
                state.is_running = False
                state.current_prompt_text = ""
    ...
    def _cmd_new(self, args: str, state: SessionState) -> str:
        """Start a new conversation: stop current turn, clear queued prompts, reset history."""
        self._cmd_stop(args, state)
        return self._cmd_reset(args, state)

    def _cmd_stop(self, args: str, state: SessionState) -> str:
        """Stop the current generation and clear queued prompts.

        Mirrors messaging-channel ``/stop`` semantics: cancel the active turn
        immediately and drop anything still waiting in the FIFO queue.
        """
        with state.runtime_lock:
            if state.is_running and state.current_prompt_text:
                state.interrupted_prompt_text = state.current_prompt_text
            ...
            try:
                ...
                logger.debug(
                    "Failed to hard-interrupt ACP session %s via /stop",
                    state.session_id,
                    exc_info=True,
                )
        return "Stopped."
